...
import { Plus, Package, Briefcase, Sparkles, Rocket, Zap, Star, ArrowRight, TrendingUp } from "lucide-react";
...
export function GigJobBuilder() {
  const { t } = useI18n();
  const { user } = useAuth();
  const { getLimit, requiresUpgrade } = useSubscription();
  ...
  return (
    <>
      <UpgradeModal
        ...
        featureName="Partnership Pitch Builder"
        featureDescription={`Ai atins limita de ${gigLimit} pitch-uri. Fă upgrade pentru a crea mai multe.`}
      />
          ...
          <div className="relative z-10">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold tracking-tight">Partnership Pitch Builder</h1>
                  <p className="text-muted-foreground text-sm">
                    Construiește propuneri de parteneriat pe baza ofertei tale.
                  </p>
            ...
            >
              <div className="bg-background/60 backdrop-blur-sm rounded-xl p-4 border border-amber-500/20">
                ...
                <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{gigs.length}</p>
                <p className="text-xs text-muted-foreground">pitch-uri</p>
              </div>

              <div className="bg-background/60 backdrop-blur-sm rounded-xl p-4 border border-blue-500/20">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-2 h-2 rounded-full bg-blue-500" />
                  <span className="text-xs text-muted-foreground uppercase tracking-wider">Job Specs</span>
                </div>
                    ...
                    <Card className="relative overflow-hidden border-2 border-transparent hover:border-primary/30 transition-all duration-300">
                      ...
                      <CardContent className="pt-6 space-y-4">
                        ...
                        >
                          ...
                          {t.gigs?.createFromPackage || "Create Pitch"}
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
                    <Card className="border-dashed">
                      <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-sm text-muted-foreground max-w-sm">Create your first pitch to start offering partnership terms</p>
                      </CardContent>
                    </Card>
                  </motion.div>
                ) : (
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
                    <Card className="border-dashed">
                      <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <h3 className="font-semibold mb-2">No job specs yet</h3>
                        <p className="text-sm text-muted-foreground max-w-sm">Create a job spec for partnership recruitment</p>
                      </CardContent>
                    </Card>
                  </motion.div>
                ) : (
