...
export default function OnboardingStep8Offer({ onOfferGenerated }: Props) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (skills.length === 0) {
        throw new Error(t.onboardingStep8.noSkillsError);
      }
      
      if (!ikigai) {
        throw new Error(t.onboardingStep8.noIkigaiError);
      }
      ...
      toast.error(error.message || t.onboardingStep8.generateError);
      setStep('idle');
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(error.message || t.onboardingStep8.saveError);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const packageConfig = [
    { key: 'starter_package', label: t.onboardingStep8.starter, icon: Zap, color: 'border-blue-500/30 bg-blue-500/5' },
    { key: 'standard_package', label: t.onboardingStep8.standard, icon: Star, color: 'border-primary/30 bg-primary/5' },
    { key: 'premium_package', label: t.onboardingStep8.premium, icon: Crown, color: 'border-yellow-500/30 bg-yellow-500/5' },
  ];
    ...
    return (
      <div className="space-y-6">
        <div className="text-center mb-8">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">{t.onboardingStep8.offerCreatedTitle}</h3>
          <p className="text-muted-foreground">
            {t.onboardingStep8.offerCreatedDescription}
          </p>
        </div>
        <div className="flex justify-center">
          ...
          >
            ...
            {t.onboardingStep8.regenerateOffer}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-6">
        <div className="text-center mb-8">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">{t.onboardingStep8.buildingTitle}</h3>
          <p className="text-muted-foreground">
            {t.onboardingStep8.buildingDescription}
          </p>
        </div>
        <Progress value={progress} className="h-2" />
        <p className="text-center text-sm text-muted-foreground">
          {progress < 30 && t.onboardingStep8.analyzingSkills}
          {progress >= 30 && progress < 60 && t.onboardingStep8.calculatingPrices}
          {progress >= 60 && progress < 90 && t.onboardingStep8.buildingPackages}
          {progress >= 90 && t.onboardingStep8.finalizing}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        <div className="text-center mb-4">
          ...
          <h3 className="text-lg font-semibold text-foreground">{t.onboardingStep8.yourOffer}</h3>
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mb-2">
                  {t.onboardingStep8.delivery} {pkg.delivery_time}
                </p>
                <ul className="space-y-1">
                  ...
                  {pkg.deliverables.length > 3 && (
                    <li className="text-xs text-primary">
                      {t.onboardingStep8.moreDeliverables.replace('{count}', String(pkg.deliverables.length - 3))}
                    </li>
                  )}
                </ul>
              </motion.div>
          ...
          >
            {isSaving ? (
              <>
                ...
                {t.onboardingStep8.savingOffer}
              </>
            ) : (
              <>
                <Check className="w-4 h-4" />
                {t.onboardingStep8.saveAndFinish}
              </>
            )}
          </Button>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        ...
        <h3 className="text-lg font-semibold text-foreground mb-2">{t.onboardingStep8.builderTitle}</h3>
        <p className="text-muted-foreground">
          {t.onboardingStep8.builderDescription}
        </p>
      </div>
      ...
      <div className="flex justify-center pt-4">
        <Button onClick={handleGenerate} size="lg" className="gap-2">
          ...
          {t.onboardingStep8.generateButton}
        </Button>
      </div>
    </div>
  );
